...
const handler = async (req, res) => {
    ...
    if (req.method == "POST") {
        ...
        if (searchUser) {
            let userId = await User.findOne({ Email: Email }, '_id').exec();
            res.status(300).json({ message: "User Already Exist!!", id: userId })
        } else {
            ...
            let userId = await User.findOne({ Email: Email }, '_id').exec();
            console.log(userId);
            res.status(200).json({ message: "Success", id: userId });
        }
    } else {
